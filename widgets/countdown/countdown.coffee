class Dashing.Countdown extends Dashing.Widget
  ...
  ready: ->
    ...
    percentage = $(@node).find(".percentage")
    meter.attr("data-bgcolor", percentage.css("background-color"))
    meter.attr("data-fgcolor", percentage.css("color"))
    meter.knob()
  ...
  startCountdown: =>
    ...
    percent = Math.round((seconds_until_end/604800)*100)
