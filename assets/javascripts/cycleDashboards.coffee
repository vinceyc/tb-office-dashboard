...
# application.coffee:
#
#         $('.grid-cycle ul:first').grid-cycle
#
# And change it to:
#
#         $('.grid-cycle > ul').grid-cycle
#
# Finally, put multiple grid-cycle divs in your dashboard, and add a call to Dashing.cycleDashboards()
...
# Customized version of `Dashing.grid-cycleLayout()` which supports multiple dashboards.
Dashing.cyclegrid-cycleLayout = (positions) ->
    #positions = positions.replace(/^"|"$/g, '') # ??
    positions = JSON.parse(positions)
    $dashboards = $(".grid-cycle > ul")
    if isArray(positions) and ($dashboards.length == positions.length)
        Dashing.customgrid-cycleLayout = true
        ...
        if !isArray(newWidgetPositions[0])
            ...
            unless JSON.stringify(newWidgetPositions) == JSON.stringify(Dashing.currentWidgetPositions)
                ...
                $('#grid-cycle-code').text("
                  <script type='text/javascript'>\n
                  $(function() {\n\n
                  \ \ Dashing.cyclegrid-cycleLayout('#{JSON.stringify(Dashing.currentWidgetPositions)}')\n
                  });\n
                  </script>
                ")
